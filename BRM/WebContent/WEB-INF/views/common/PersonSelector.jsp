...
$(document).ready(function(){
	...
	$("#list").jqGrid({
		datatype : 'local',
		colNames: ['id','员工编号','姓名', '岗位'],
		colModel: [{name:'employeeId',hidden:true,key:true},
			   		{name:'employeeNum',width:125,sortable:false},
...
function delPerson(obj){
	$.each(arr, function(i, o){
		if(o.employeeId == obj.employeeId) {
			arr.splice(i, 1);
			return false;
		}
	});
...
function ok(){
	...
	var idx = "${idx}";
	if(idx == "")
		parent.getParentWin().setPersonSelectorVal(ids, names, nums);
	else
		parent.getParentWin().setPersonSelectorVal(idx, ids, names, nums);
	parent.closeFrame();
}
</script>
</head>
