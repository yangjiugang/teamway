...
<body>
	<div class="HR_ZZJL">
	<c:if test="${fn:length(edusByType) == 0}">
		<p>
			从<input class="NYinput" value="${edu.eduStartDate }" readonly="readonly"/>
			到<input class="NYinput"  value="${edu.eduEndDate }" readonly="readonly"/>
		</p>
		<p>
			培训机构名称：<input class="XXMCinput" value="${edu.schoolName }" readonly="readonly"/>
			专业：<input class="XXMCinput" value="${edu.profession }" readonly="readonly"/>
		</p>
	</c:if>
	<c:forEach items="${edusByType}" var="edu" varStatus="status">
		<p>
			从<input class="NYinput" value="${edu.eduStartDate }" readonly="readonly"/>
			到<input class="NYinput"  value="${edu.eduEndDate }" readonly="readonly"/>
		</p>
