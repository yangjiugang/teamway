...
Popup.prototype=
{
	...
	/*	G('dialogCase')?G('dialogCase').parentNode.removeChild(G('dialogCase')):function ()
		...
		oDiv.id='dialogCase'+n;
		document.body.appendChild(oDiv);
		
	},build:function ()
	{
		var baseZIndex=10001+this.info.overlay*10+n;
		var showZIndex=baseZIndex+2;
		this.iframeIdName='ifr_popup'+this.info.overlay+n;
		var close='<input type="image" id="dialogBoxClose'+n+'" src="js/popup/dialogclose.gif" border="0" width="11" height="11" align="absmiddle" title="关闭"/>';
		var cB='filter: alpha(opacity='+this.info.coverOpacity+');opacity:'+this.info.coverOpacity/100+';';
		var cover='<div id="dialogBoxBG'+n+'" style="position:absolute;top:0px;left:0px;width:100%;height:100%;z-index:'+baseZIndex+';'+cB+'background-color:'+this.color.cColor+';display:none;"></div>';
		var mainBox='<div id="dialogBox'+n+'" style="border:1px solid '+this.color.tColor+';display:none;z-index:'+showZIndex+';position:relative;width:'+this.config.width+'px;"><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="'+this.color.bColor+'">';
		if(this.config.isHaveTitle)
		{
			mainBox+='<tr style="background:url(js/popup/top.gif) left top repeat-x;" height="27" bgcolor="'+this.color.tColor+'"><td><table style="-moz-user-select:none;height:24px;" width="100%" border="0" cellpadding="0" cellspacing="0" ><tr>'+'<td width="6" height="24"></td><td id="dialogBoxTitle'+n+'" style="color:'+this.color.wColor+';font-size:14px;font-weight:bold;">'+this.info.title+'&nbsp;</td>'+'<td id="dialogClose'+n+'" width="20" valign="middle">'+close+'</td><td width="6"></td></tr></table></td></tr>'
		}else 
		{
			mainBox+='<tr height="10"><td align="right">'+close+'</td></tr>'
		};
		mainBox+='<tr style="height:'+this.config.height+'px" valign="top"><td id="dialogBody'+n+'" style="position:relative;"></td></tr></table></div>'+'<div id="dialogBoxShadow'+n+'" style="display:none;z-index:'+baseZIndex+';"></div>';
		if(!this.config.isBackgroundCanClick)
		{
			G('dialogCase'+n).innerHTML=cover+mainBox;
			this.setBackgroundSize();
			window.onresize=this.setBackgroundSize;
			
		}else 
		{
			G('dialogCase'+n).innerHTML=mainBox;
			
		}
		Event.observe(G('dialogBoxClose'+n),"click",this.reset.bindAsEventListener(this),false);
		if(this.config.isSupportDraging)
		{
			dropClass=new Dragdrop(this.config.width,this.config.height,this.info.shadowWidth,this.config.isSupportDraging,this.config.contentType);
			G("dialogBoxTitle"+n).style.cursor="move"
		};
		this.lastBuild()
	},setBackgroundSize:function ()
	{
		...
		G('dialogBoxBG'+n).style.width=getValueWidth;
		var getValueHeight;
		var getMaxValueHeight=[getValue("clientHeight"),getValue("scrollHeight")];
		getValueHeight=eval("Math.max("+getMaxValueHeight.toString ()+")");
		G('dialogBoxBG'+n).style.height=getValueHeight;
	...
	},lastBuild:function ()
	{
		...
		{
			var openIframe="<iframe width='100%' style='overflow-y:auto;height:"+this.config.height+"px' name='"+this.iframeIdName+"' id='"+this.iframeIdName+"' src='"+this.info.contentUrl+"' frameborder='0' scrolling='"+this.config.scrollType+"'></iframe>";
			var coverIframe="<div id='iframeBG"+n+"' style='position:absolute;top:0px;left:0px;width:1px;height:1px;z-index:"+coverIfZIndex+";filter: alpha(opacity=00);opacity:0.00;background-color:#ffffff;'><div>";
			G("dialogBody"+n).innerHTML=openIframe+coverIframe;
			
		}else if(this.config.contentType==2)
		{
			G("dialogBody"+n).innerHTML=this.info.contentHtml;
			
		}else if(this.config.contentType==3)
		{
			G("dialogBody"+n).innerHTML=confirm;
			Event.observe(G('dialogOk'),"click",this.forCallback.bindAsEventListener(this),false);
			Event.observe(G('dialogCancel'),"click",this.close.bindAsEventListener(this),false);
			
		}else if(this.config.contentType==4)
		{
			G("dialogBody"+n).innerHTML=alert;
	...
	{
		G('dialogBody'+n).height=G('dialogBody'+n).clientHeight;
		this.lastBuild()
	},show:function ()
	{
		this.hiddenSome();
	...
	{
		var oShadow=G('dialogBoxShadow'+n);
		var oDialog=G('dialogBox'+n);
	...
	{
		if(!this.config.isBackgroundCanClick)G('dialogBoxBG'+n).style.display='';
		var oDialog=G('dialogBox'+n);
	...
	},close:function ()
	{
		...
		G('dialogBox'+n).style.display='none';
		if(!this.config.isBackgroundCanClick)G('dialogBoxBG'+n).style.display='none';
		if(this.config.isShowShadow)G('dialogBoxShadow'+n).style.display='none';
		G('dialogBody'+n).innerHTML='';
		this.showSome();
		
		var a = document.getElementById('dialogCase'+n);
		var p = a.parentNode;
		p.removeChild(a);
...
Dragdrop.prototype=
{
	initialize:function (width,height,shadowWidth,showShadow,contentType)
	{
		...
		this.oObj=G('dialogBox'+n);
		Event.observe(G('dialogBoxTitle'+n),"mousedown",this.moveStart.bindAsEventListener(this),false);
	...
	},moveStart:function (event)
	{
		...
		{
			G("iframeBG"+n).style.display="";
			G("iframeBG"+n).style.width=this.width;
			G("iframeBG"+n).style.height=this.height;
	...
	},mousemove:function (event)
	{
		...
		if(this.showShadow)
		{
			G('dialogBoxShadow'+n).style.left=iLeft+this.shadowWidth+"px";
			G('dialogBoxShadow'+n).style.top=iTop+this.shadowWidth+"px";
	...
	{
		if(!this.IsDraging)return ;
		if(this.contentType==1)G("iframeBG"+n).style.display="none";
		...
		if(mousX<1||mousY<1||mousX>document.body.clientWidth||mousY>document.body.clientHeight||parseInt(this.oObj.style.left)<0||parseInt(this.oObj.style.top)<0||oObjRight>document.body.clientWidth||oObjBottom>document.body.clientHeight)
		{
			...
			if(this.showShadow)
			{
				G('dialogBoxShadow'+n).style.left=this.backData.x+this.shadowWidth;
				G('dialogBoxShadow'+n).style.top=this.backData.y+this.shadowWidth;
...
function remove(){
	G('dialogBox'+(n-3)).style.display='none';
	G('dialogBoxBG'+(n-3)).style.display='none';
	G('dialogBoxShadow'+(n-3)).style.display='none';
	G('dialogBody'+(n-3)).innerHTML='';
	this.showSome();
	
	var a = document.getElementById('dialogCase'+(n-3));
	var p = a.parentNode;
	p.removeChild(a);
}
function getChildFrame(){
	var id = 'ifr_popup0'+(n-3);
	if(document.getElementById(id) != null)
	 	return document.getElementById(id).contentWindow;
	 return null;
}

function getChildObj(id){
	if(window.frames['ifr_popup03'].document != null){
		return window.frames['ifr_popup03'].document.getElementById(id); 					// IE
	}else{
		return document.getElementById('ifr_popup03').contentDocument.getElementById(id);	// Firefox
	}
}