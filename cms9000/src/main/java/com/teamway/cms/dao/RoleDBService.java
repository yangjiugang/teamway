...
public interface RoleDBService extends QueryRepository {
	...
	//查询所有的摄像机信息
	@Query("select * from cms_res_camera_info where code in (?1)")
	List<Cms_res_camera_infoEntity> findAllCameraByCode(List<String> codes);
	
	
	
	//查询所有的预置位信息
	@Query("select * from cms_res_camera_preset_info where camera_code in (?1)")
	List<Cms_res_camera_preset_infoEntity> findAllPresetByCodes(List<String> codes);
