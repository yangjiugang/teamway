...
<body>
	...
	<div class="middle">
		...
		<div class="FLHRight">
			<!---Y01首页搜索结果--->
			<span class="jiangjie">符合搜索条件的内容<span class="red">${size}</span>条
			</span>
			<ul class="Y01Result">
				<li>姓名</li>
				<li>联系电话</li>
				...
				<li style="width: 150px;">看房次数</li>
				<li></li>
			</ul>

			<c:forEach items="${lookFangList}" var="list">
				<ul class="Y01jg">
					<li>${list.name}</li>
					<li>${list.cellphone}</li>
					<li style="width: 150px;">${list.address}</li>
					<li style="width: 150px;">
					<c:if test="${list.customerType == 1}">预备客户</c:if>
					<c:if test="${list.customerType == 2}">普通客户</c:if>
					<c:if test="${list.customerType == 3}">会员客户</c:if>
					</li>
					<li style="width: 150px;">${list.countEvent}</li>
					<li><a
						href="IndexBEE?nameXiangqing=${list.cusId}"
						class="blue">详情</a></li>
				</ul>
			</c:forEach>
			<!---End Y01首页搜索结果--->
			<ul class="Y01-tong">
				<li>统计：<span class="red">${size}</span>条
				</li>
			</ul>
	...
	<div id="sampleformdiv"
		style="display: none; width: 300px; height: 500px;" title="更换图像">
		在电脑上选中一张图片<br />
		<form method="post" id="sampleform" name="sampleform"
			action="changeTouXiang">
			<input type="file" name="fileUp" /><br> <input type="submit"
				name="sub" value="修改" /> <input type="reset" name="reset"
				value="重新选择" />
		</form>
		<br>
		<input type="button" value="退出" id="exit" style="float: right;">
	</div>
</body>
<script type="text/javascript">
	$(document).ready(function() {
