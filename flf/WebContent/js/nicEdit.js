...
var nicEditorConfig = bkClass.extend({
	buttons : {
		...
		'justify' : {name : __('默认'), command : 'justifyfull', noActive : true},
		...
		'space1': {name : __(''), command : '', noActive : true},
		'space2': {name : __(''), command : '', noActive : true}
	},
	iconsPath : 'images/nicEditorIcons.gif',
	buttonList : ['save','bold','italic','underline','left','center','right','justify','ol','ul','fontSize','fontFamily','fontFormat','indent','outdent','image','upload','link','unlink','forecolor','bgcolor'],
	iconList : {"xhtml":1,"bgcolor":2,"forecolor":3,"bold":4,"center":5,"hr":6,"indent":7,"italic":8,"justify":9,"left":10,"ol":11,"outdent":12,"removeformat":13,"right":14,"saves":25,"undo":15,"strikethrough":16,"subscript":17,"superscript":18,"ul":19,"underline":20,"image":21,"link":22,"unlink":23,"close":24,"arrow":26,"upload":27}
...
// 自定义保存
var nicCustomSaveOptions = {
	buttons : {
		'saves' : {name : ('保存'), type : 'nicEditorSaveSelect'}
	}
};
// 自定义撤销
var nicCustomUndoOptions = {
	buttons : {
		'undo' : {name : ('撤销'), type : 'nicEditorUndoSelect'}
	}
};
// 保存按钮
var nicEditorSaveSelect = nicEditorButton.extend({
	mouseClick: function(){
		save();
	}
});
...
// 撤销按钮
var nicEditorUndoSelect = nicEditorButton.extend({
	mouseClick: function(){	
...
nicEditors.registerPlugin(nicPlugin,nicCustomSaveOptions);
nicEditors.registerPlugin(nicPlugin,nicCustomUndoOptions);
...
var nicEditorFontFamilySelect = nicEditorSelect.extend({
	sel : {'Simson' : '宋体','Simhei' : '黑体','仿宋_GB2312' : '仿宋_GB2312','楷体_GB2312' : '楷体_GB2312', '隶书' : '隶书', 'Microsoft Yahei' : '微软雅黑', 'Arial' : 'Arial', 'Calibri' : 'Calibri', 'Tahoma' : 'Tahoma','Helvetica':'Helvetica','Verdana':'Verdana'},
	
	init : function() {
		this.setDisplay('选择字体');
		for(itm in this.sel) {
			this.add(itm,'<font size=2 face="'+itm+'">'+this.sel[itm]+'</font>');
		}
	}
});
...
var nicColorOptions = {
	buttons : {
		'forecolor' : {name : __('设置文本颜色'), type : 'nicEditorColorButton', noClose : true},
...
//		'image' : {name : '添加图片', type : 'nicImageButton', tags : ['IMG']}
		'image' : {name : '添加图片', type : 'imageBox', tags : ['IMG']}
	}
...
/* 弹出图片上传框 */
var imageBox = nicEditorButton.extend({
	mouseClick: function(){
		$("#url").val('');
		$("#fileQueue").html('');
		uploadImageWin();
